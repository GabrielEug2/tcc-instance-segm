...
def compute_classmap(ann_file: Path):
	...
	Args:
		ann_file (str): path to an annotation file, in COCO format.

	Raises:
		FileNotFoundError: if the requested file was not found.
		ValueError: if the annotations does not follow the expected format.
	...
	try:
		...
		for cat in anns["categories"]:
			default_coco_map[str(cat['id'])] = cat['name']
