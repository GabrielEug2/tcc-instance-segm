...
class PredManager:
	...
	def save(self, predictions: list[Prediction], img_name: str, model_name: str):
		...
		out_file = self.root_dir / model_name / f'{img_name}.json'
		out_file.parent.mkdir(parents=True, exist_ok=True)
		with out_file.open('w') as f:
			json.dump(serializable_preds, f, indent=4)

	def load(self, img_name: str, model_name: str) -> list[Prediction]:
		pred_file = self.root_dir / model_name / f'{img_name}.json'
		try:
	...
	def get_n_images_with_predictions(self, model_name: str) -> int:
		...
		for img_name in self._get_img_names_for_model(model_name):
			predictions = self.load(img_name, model_name)
			if len(predictions) >= 1:
				n_images_with_preds += 1

		return n_images_with_preds

	def get_n_objects(self, model_name: str, img_name: str = None) -> int:
		if img_name is None:
			return sum(self.class_distribution(model_name).values())
		else:
			return sum(self.class_distribution(model_name, img_name=img_name).values())

	def class_distribution(self, model_name: str, img_name: str = None) -> dict[str, int]:
		if img_name is None and hasattr(self, '_cached_class_dists') and model_name in self._cached_class_dists:
			return self._cached_class_dists[model_name]

		class_dist = None
		if img_name is None:
			class_dist = self._class_dist_on_all_imgs(model_name)

			if not hasattr(self, '_cached_class_dists'):
				self._cached_class_dists = {}
			self._cached_class_dists[model_name] = class_dist
		else:
			class_dist = self._class_dist_on_img(model_name, img_name)
		
		return class_dist

	def _class_dist_on_all_imgs(self, model_name: str) -> dict[str, int]:
		class_dist = defaultdict(lambda: 0)
		for img_name in self._get_img_names_for_model(model_name):
			class_dist_on_img = self._class_dist_on_img(model_name, img_name)
	...
	def _class_dist_on_img(self, model_name: str, img_name: str) -> dict[str, int]:
		predictions = self.load(img_name, model_name)
	...
	def filter(self, out_dir: Path, model_name: str, classes: list[str] = None, img_name: str = None):
		...
		if classes is not None:
			...
			self._filter_by_img(model_name, img_name, filtered_pred_manager)

	def normalize_classnames(self):
		for model in self.get_model_names():
			for img in self._get_img_names_for_model(model):
	...
	def _filter_by_classes(self, model_name: str, classes: list[str], filtered_pred_manager: 'PredManager'):
		for img_name in self._get_img_names_for_model(model_name):
			predictions_for_img = self.load(img_name, model_name)

			filtered_preds = [p for p in predictions_for_img if p.classname in classes]

			filtered_pred_manager.save(filtered_preds, img_name, model_name)

	def _filter_by_img(self, model_name: str, img_name: str, filtered_pred_manager: 'PredManager'):
		filtered_preds = self.load(img_name, model_name)

		filtered_pred_manager.save(filtered_preds, img_name, model_name)

	def _get_img_names_for_model(self, model_name: str) -> Generator:
