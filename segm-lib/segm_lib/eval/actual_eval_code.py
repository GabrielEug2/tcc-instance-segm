...
from segm_lib.eval.structures.model_results import DatasetResults, ImageResults
...
def evaluate_on_dataset(eval_files: EvalFiles, model_name: str):
	...
	for k, k0 in enumerate(k_list):
		...
		# with id 2 (again, it's not *quite* like that, as I explain in
		...
		with HiddenPrints():
			...
			for k, k0 in enumerate(k_list):
				...
				for gtId, dtId in zip(evalImg_for_that_k['gtIds'], gtm_per_threshold[0]):
					...
					if dtId == 0:
						# false negative
						fns_on_img.append(gt_in_custom_format)

			n_anns_considered = AnnManager(eval_files.per_image[img_name].custom_anns_dir).get_n_objects()
			n_preds_considered = PredManager(eval_files.per_image[img_name].custom_preds_dir).get_n_objects(model_name)
